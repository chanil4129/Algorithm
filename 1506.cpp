...
int main(void) {
	...
	for (int i = 0; i < N; i++) {
		cin >> input;
		for (int j = 0; j < N; j++) city[i][j] = input[j] - '0';		//붙여서 입력하므로 값을 따로 받아줘야함
	}
	//플로이드 알고리즘을 통해 경로 파악
	for (int k = 0; k < N; k++) for (int i = 0; i < N; i++) for (int j = 0; j < N; j++) if (city[i][k] && city[k][j]) city[i][j] = 1;
	for (int i = 0; i < N; i++) if (!visited[i]) {		//방문하지 않은 집단일때만 실행(방문 안했으면 0)
		int k = cost[i];		//최소 비용
		for (int j = 0; j < N; j++) if (city[i][j] & city[j][i]) {		//왔다갔다 할 수 있는 도시들만 하나의 집단이므로 그때만 실행
